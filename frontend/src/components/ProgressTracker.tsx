  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-300">{status.current_step}</p>
      </div>
        ...
        {['planning', 'executing', 'publishing', 'done'].map((stage, index) => (
          <div key={stage} className="flex items-center">
            <div className="flex flex-col items-center">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center ${status.status === stage ||
      ...
      {subQuestions && subQuestions.length > 0 && (
        <div className="space-y-3">
          <h4 className="font-semibold text-gray-900 dark:text-gray-100">Research Questions</h4>
          <div className="space-y-2">
            {subQuestions.map((q, index) => (
              <div
                key={index}
                className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 border border-gray-200 dark:bg-gray-800 dark:border-gray-700"
              >
